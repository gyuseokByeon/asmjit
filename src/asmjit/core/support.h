...
namespace Internal {
  ...
  ASMJIT_INLINE T addOverflowFallback(T x, T y, FastUInt8* of) noexcept {
    ...
    U result = U(x) + U(y);
    *of = FastUInt8(*of | (isUnsigned<T>() ? result < U(x) : T((U(x) ^ ~U(y)) & (U(x) ^ result)) < 0));
    return T(result);
  }
  ...
  ASMJIT_INLINE T mulOverflowFallback(T x, T y, FastUInt8* of) noexcept {
    ...
    if (std::is_signed<T>::value) {
      U prod = U(I(x)) * U(I(y));
      *of = FastUInt8(*of | (I(prod) < I(std::numeric_limits<T>::lowest()) || I(prod) > I(std::numeric_limits<T>::max())));
      return T(I(prod & mask));
    }
    else {
      U prod = U(x) * U(y);
      *of = FastUInt8(*of | ((prod & ~mask) != 0));
      return T(prod & mask);
    }
  }
  ...
  ASMJIT_INLINE uint64_t mulOverflowFallback(uint64_t x, uint64_t y, FastUInt8* of) noexcept {
    uint64_t result = x * y;
    *of = FastUInt8(*of | (y != 0 && allOnes<uint64_t>() / y < x));
    return result;
  }
