...
Error RAPass::runOnFunction(Zone* zone, Logger* logger, FuncNode* func) {
  ...
  // TODO: Only needed when a single VirtReg is used across multiple functions.
  //       The question is what to do with that. Should it be even allowed?
...
static void RAPass_dumpRAInst(RAPass* pass, String& sb, const RAInst* raInst) noexcept {
  ...
  for (uint32_t i = 0; i < tiedCount; i++) {
    ...
    if (tiedReg.isLast()) sb.append("|Last");
    if (tiedReg.isKill()) sb.append("|Kill");
