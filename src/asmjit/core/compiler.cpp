...
Error BaseCompiler::_newFuncNode(FuncNode** out, const FuncSignature& signature) {
  *out = nullptr;
  ...
  Error err = funcNode->detail().init(signature);
  ...
  if (funcNode->_funcDetail._callConv.naturalStackAlignment() < _codeInfo.stackAlignment())
    funcNode->_funcDetail._callConv.setNaturalStackAlignment(_codeInfo.stackAlignment());
...
Error BaseCompiler::_newInvokeNode(InvokeNode** out, uint32_t instId, const Operand_& o0, const FuncSignature& signature) {
  ...
  Error err = node->detail().init(signature);
  if (ASMJIT_UNLIKELY(err))
    return reportError(err);
