...
void logInstructionEmitted(
  ...
  Formatter::formatInstruction(sb, flags, self, self->archId(), BaseInst(instId, options, self->extraReg()), opArray, Globals::kMaxOpCount);
...
Error logInstructionFailed(
  ...
  Formatter::formatInstruction(sb, 0, self, self->archId(), BaseInst(instId, options, self->extraReg()), opArray, Globals::kMaxOpCount);
