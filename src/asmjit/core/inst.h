...
struct InstRWInfo {
  ...
  //!
  //! TODO: Flags are not used at the moment, they are always zero.
  //!
  //! \{
...
ASMJIT_API Error validate(uint32_t archId, const BaseInst& inst, const Operand_* operands, size_t opCount) noexcept;
#endif // !ASMJIT_NO_VALIDATION

#ifndef ASMJIT_NO_INTROSPECTION
//! Gets Read/Write information of the given instruction.
ASMJIT_API Error queryRWInfo(uint32_t archId, const BaseInst& inst, const Operand_* operands, size_t opCount, InstRWInfo* out) noexcept;

//! Gets CPU features required by the given instruction.
ASMJIT_API Error queryFeatures(uint32_t archId, const BaseInst& inst, const Operand_* operands, size_t opCount, BaseFeatures* out) noexcept;
