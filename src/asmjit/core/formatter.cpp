...
#ifdef ASMJIT_BUILD_X86
  if (ArchInfo::isX86Family(archId))
    return x86::FormatterInternal::formatRegister(sb, formatFlags, emitter, archId, regType, regId);
#endif

#ifdef ASMJIT_BUILD_ARM
  if (ArchInfo::isArmFamily(archId))
    return arm::FormatterInternal::formatRegister(sb, formatFlags, emitter, archId, regType, regId);
#endif
...
#ifdef ASMJIT_BUILD_X86
  if (ArchInfo::isX86Family(archId))
    return x86::FormatterInternal::formatOperand(sb, formatFlags, emitter, archId, op);
#endif

#ifdef ASMJIT_BUILD_ARM
  if (ArchInfo::isArmFamily(archId))
    return arm::FormatterInternal::formatOperand(sb, formatFlags, emitter, archId, op);
#endif
...
#ifdef ASMJIT_BUILD_X86
  if (ArchInfo::isX86Family(archId))
    return x86::FormatterInternal::formatInstruction(sb, formatFlags, emitter, archId, inst, operands, opCount);
#endif

#ifdef ASMJIT_BUILD_ARM
  if (ArchInfo::isArmFamily(archId))
    return arm::FormatterInternal::formatInstruction(sb, formatFlags, emitter, archId, inst, operands, opCount);
#endif
...
static Error formatFuncValue(String& sb, uint32_t formatFlags, const BaseEmitter* emitter, FuncValue value) noexcept {
  ...
  if (value.isReg()) {
    ASMJIT_PROPAGATE(sb.append('@'));
    ASMJIT_PROPAGATE(formatRegister(sb, formatFlags, emitter, emitter->archId(), value.regType(), value.regId()));
  }
...
Error formatNode(
  ...
  switch (node->type()) {
    ...
    case BaseNode::kNodeLabel: {
      ...
      ASMJIT_PROPAGATE(formatLabel(sb, formatFlags, builder, labelNode->id()));
      ASMJIT_PROPAGATE(sb.append(":"));
      break;
    }
    ...
    case BaseNode::kNodeEmbedLabel: {
      ...
      ASMJIT_PROPAGATE(formatLabel(sb, formatFlags, builder, embedNode->id()));
      break;
    }
    ...
    case BaseNode::kNodeEmbedLabelDelta: {
      ...
      ASMJIT_PROPAGATE(formatLabel(sb, formatFlags, builder, embedNode->id()));
      ASMJIT_PROPAGATE(sb.append(" - "));
      ASMJIT_PROPAGATE(formatLabel(sb, formatFlags, builder, embedNode->baseId()));
      ASMJIT_PROPAGATE(sb.append(")"));
      break;
    }
    ...
    case BaseNode::kNodeFunc: {
      ...
      ASMJIT_PROPAGATE(formatLabel(sb, formatFlags, builder, funcNode->id()));
      ASMJIT_PROPAGATE(sb.append(": "));
    ...
    case BaseNode::kNodeFuncRet: {
      ...
      for (uint32_t i = 0; i < 2; i++) {
        ...
        if (!op.isNone()) {
          ...
          ASMJIT_PROPAGATE(formatOperand(sb, formatFlags, builder, builder->archId(), op));
        }
      }
      break;
    }
